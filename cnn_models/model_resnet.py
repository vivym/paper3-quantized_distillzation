...
class ResNet(nn.Module):
    def __init__(self, block, layers, network_type, num_classes, att_type=None):
        ...
        if network_type == "small":
            ...
            self.layer2 = self._make_layer(block, 64,shape//2, layers[1], stride=2, att_type=att_type, no_spatial=False)
            self.layer3 = self._make_layer(block, 64, shape//4,layers[2], stride=2, att_type=att_type, no_spatial=False)
            self.layer4 = self._make_layer(block, 64, shape//8, layers[3], stride=2, att_type=att_type, no_spatial=False)  
...
def ResidualNet(network_type, depth, num_classes, att_type):
    ...
    assert depth in ["small", 18, 34, 50, 101], 'network depth should be 18, 34, 50 or 101'

    if depth == "small":
        model = ResNet(BasicBlock, [1, 1, 1, 1], network_type, num_classes, att_type)
